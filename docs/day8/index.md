...
* Write code that processes `raw` documents one by one,
  annotates temporal expressions in each of them (using the regular
  expressions made in 1.1) and output the result into a new
  folder.
...
* Write a program that process raw documents one by one,
  annotates them with temporal expressions using spaCy pattern matcher and
  outputs the results to another folder.

Use patterns made in Exercise 2.1. The documents in
the new folder should be named `<no>_sub.txt`, where `<no>` is the
same document number as in `raw` folder. (You can re-use your code
from above for the file processing.)
